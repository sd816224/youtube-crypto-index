...
def fetch_videos(google_api_key,playlistId,maxResults,page_token=None):
   ...
   """
    ...
    payload=fetch_videos_page(google_api_key,playlistId,maxResults,page_token)
    next_page_token=payload.get('nextPageToken')
    all_items=payload['items']
    page_no=2
    while next_page_token:
        logger.info(f'fetch page: {page_no}')
        payload=fetch_videos_page(google_api_key,playlistId,maxResults,next_page_token)
        next_page_token=payload.get('nextPageToken')
        all_items+=payload['items']
        page_no+=1
...
# maxResults='50'
# videos=fetch_videos(google_api_key,playlistId,maxResults)
# save_channels(videos)
# pprint(len(videos['items']))